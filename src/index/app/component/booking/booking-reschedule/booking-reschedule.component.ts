...
@ViewChild('btnIncludeServices') btnIncludeServices:any;
  ngOnInit(): void {
    ...
    this.title.setTitle(" Mirrors Beauty Lounge | Reshedule"); // <-- Update the title 
    let model={PageName:'Booking reschedule',Title:'Booking reschedule',IsSearch:false,IsBackButton:false}
...
if(IsValid==1){
  ...
  this.categoryService.CancelOrder(model).subscribe((data:any)=>{
    ...
    this.sharedService.AddLog('booking-reschedule','Cancelled='+this.orderId);
      let sms={
...
this.IsLoading=true;
  ...
  this.categoryService.Reschedule(model).subscribe((data:any)=>{
    ...
    this.sharedService.AddLog('booking-reschedule','Reshedule='+this.orderId);
      this.categoryService.SMS(sms).subscribe((x:any)=>{
      this.sharedService.Message(data[0].msg,'success');
    this.btnRescheduleClose.nativeElement.click();
    this.btnCancelBooking.nativeElement.click();
