...
@Component({
  selector: 'app-popup-date-slot',
  templateUrl: './popup-date-slot.component.html',
  styleUrls: ['./popup-date-slot.component.css']
})
export class PopupDateSlotComponent implements OnInit {
  slotDate:any;
  slotTime:any;
  ...
  @Output() editedEvent = new EventEmitter<String>();
    ...
    GetSlotDateList(days:number){
      ...
      this.getSlotDate(this.slotDate[0],1);
      //this.GetSelectedSlot();
    }
    
    
    getSlotDate(data:any,IsFirstTime:any){
   ...
   let rounded = new Date(Math.round(selectedDate.getTime() / coeff) * coeff)
    ...
    if(IsFirstTime==1){
      if(this.slotTime.length==0){
        this.slotDate=this.slotDate.filter((item:any) => item !== data)
        this.getSlotDate(this.slotDate[0],0);
      }
    }
